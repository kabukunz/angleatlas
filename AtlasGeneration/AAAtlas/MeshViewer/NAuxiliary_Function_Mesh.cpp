...
void NInteractiveViewerWidget::inverse_mesh_connectivity()
{
	TimeProfiler t_process;
	t_process.startedProcess("inverse mesh connectivity");
	Mesh temp_mesh;
	...
	t_process.finishedProcess("inverse mesh connectivity", true);
}

void NInteractiveViewerWidget::scale_mesh_using_BBox(int max_len)
{
	TimeProfiler t_process;
	t_process.startedProcess("scale mesh");
	...
	t_process.finishedProcess("scale mesh", true);
}

void NInteractiveViewerWidget::split_quad_mesh()
{
	if (meshMode() == QUAD)
	{
		TimeProfiler t_process;
		t_process.startedProcess("split quad mesh");
		Mesh temp_mesh;
		...
		t_process.finishedProcess("split quad mesh", true);
	}
}

void NInteractiveViewerWidget::transform_mesh(const std::vector<double>& m)
{
...
void NInteractiveViewerWidget::geneate_rect_mesh(const std::vector<double>& p)
{
	TimeProfiler t_process;
	t_process.startedProcess("generate rect mesh");
	Mesh temp_mesh;

	t_process.finishedProcess("generate rect mesh", true);
}

void NInteractiveViewerWidget::find_vertex_by_id(int id)
{
...
void NInteractiveViewerWidget::find_vertex_by_valance(int valance)
{
	if (valance > 0)
	{
...
void NInteractiveViewerWidget::find_face_by_id(int id)
{
...
void NInteractiveViewerWidget::find_edge_by_id(int id)
{
...
void NInteractiveViewerWidget::delete_vertex_valence_four()
{
}

void NInteractiveViewerWidget::delete_vertex_valence_three()
{
}

void NInteractiveViewerWidget::split_vertex_valence_eight()
{
}